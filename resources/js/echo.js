...
window.Echo.channel("chat-channel")
    .listen(".message.sent", function (event) {
        ...
        if (parseInt(event.message.user_id) !== parseInt(userId)) {
            messagesDiv.append(
                `<p><strong class="text-green-600">${event.message.user.name}:</strong> ${event.message.message} <span style="font-size:10px">${formatDate(event.message.created_at)}</span></p>`
            );
        }
    });
